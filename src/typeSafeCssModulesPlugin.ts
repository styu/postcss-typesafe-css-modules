...
export function typeSafeCssModulesPlugin(
  ...
  return postcssModules({
    ...
    getJSON: async (from, exportTokens, to) => {
      if (to == null) {
        throw new Error("Could not determine CSS module output file location");
      }
      ...
      const dts = new SourceNode(null, null, sourceFile, [
        ...classes.flatMap((el) => {
          return [
            new SourceNode(null, null, sourceFile, `export declare const `),
            new SourceNode(
              el.location?.line ?? null,
              el.location?.column ?? null,
              sourceFile,
      ...
      await Promise.all([
        ...
        fs.outputFile(
          path.join(path.dirname(to), `${path.basename(from)}.js.map`),
          js.map.toString(),
        ),
        fs.outputFile(
        ...
        fs.outputFile(
          path.join(path.dirname(to), `${path.basename(from)}.d.ts.map`),
          dts.map.toString(),
        ),
      ]);
    },
